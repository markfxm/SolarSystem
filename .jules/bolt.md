## 2026-08-18 - [Zero-Allocation Spatial Indexing in Player Collision Physics]
**Learning:** Generating template string keys (`${x},${z}`) and temporary `Set` / `Array` containers inside high-frequency 60fps character collision query loops (e.g., `PlayerCollision.js`) creates substantial garbage collection pressure. Encoding spatial cell coordinates into a single primitive float integer `(x + 2000000) * 4000000 + (z + 2000000)` and populating a reusable instance-scoped `_scratchBoxSet` eliminates heap allocations entirely during movement and collision checks. Furthermore, evaluating `distanceSq >= radiusSq` first completely bypasses `Math.sqrt` calls for non-colliding AABBs.
**Action:** Use primitive numeric keys for spatial hash maps and reuse instance-scoped scratch sets to achieve zero-allocation physics collision checks. Always perform squared-distance bounds checks before calculating square roots.
...
**Learning:** Mutating and returning a single module-scoped, non-reactive object or array reference inside a Vue `computed` property can break reactivity when consumers track the computed value itself. Vue's reactivity system uses `Object.is()` identity comparison for the computed result, so returning the same non-reactive reference can prevent Vue from notifying downstream template nodes and watchers when underlying reactive dependencies (such as language/i18n state) change. Reactive containers created with `reactive()` may still notify consumers through property-level tracking.
**Action:** When a `computed` property transforms data into a non-reactive object or array and its consumers track the computed value itself, return a new container so the result identity changes when its contents change. Rely on Vue's dependency tracking to re-evaluate the computed property only when its dependencies change.

## 2026-05-23 - [Short-Circuit Spring Physics in 60fps Instanced Loops]
**Learning:** In 60fps instanced animation loops with pointer interaction, evaluating spring physics equations and typed-array updates for all instances every frame incurs significant overhead when the vast majority (~99%) of instances have zero velocity and offset. Short-circuiting the calculation when pointer velocity, offset, and impulse are zero avoids ~9 floating-point multiplications and array mutations per instance per frame. Snapping residual spring oscillations (< 1e-5) back to zero allows settled instances to immediately re-enter the fast path.
**Action:** In high-frequency instanced physics loops, check if local offset/velocity variables are zero before executing spring integration equations, and snap tiny residual values to zero when motion settles.
