...
**Learning:** In Vue 3 Single File Components (SFCs), variables and static structures declared within `<script setup>` are compiled inside the component's `setup()` lifecycle method. As a result, they are re-instantiated and re-allocated every time a component instance is mounted. Extracting static configuration arrays (like slider presets) and heavy objects (like `Intl.NumberFormat`) to a separate module-level `<script>` block ensures they are instantiated exactly once at module load, drastically reducing memory footprint and setup overhead.
**Action:** Always hoist invariant configurations, static arrays, and constructor-heavy instances (e.g. formatters, default options) from `<script setup>` into a module-scoped `<script>` block.
